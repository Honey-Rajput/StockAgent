...
def scan_cupid(symbol: str, df_weekly: pd.DataFrame, market_cap_cr: float = 0.0) -> dict | None:
    """
    Scans weekly OHLCV data for the 'Cupid - King of the Charts' strategy.
    
    Phases:
    1. Base: Long tight consolidation.
    2. Turn/Volume: Recent volume surge (smart money footprint) and price crossing above 10W EMA and 40W SMA.
    """
    if df_weekly is None or len(df_weekly) < 45:
        return None
    ...
    try:
        ...
        # Moving Averages: 10-week EMA and 40-week SMA
        ema10 = close.ewm(span=10, adjust=False).mean()
        sma40 = close.rolling(window=40).mean()
        
        ema10_val = float(ema10.iloc[-1])
        sma40_val = float(sma40.iloc[-1])
        
        if pd.isna(sma40_val):
            return None
            
        # Base Phase Check (Last 24 weeks before the recent 2 weeks)
        if len(close) < 26:
            return None
        
        base_high = float(high.iloc[-26:-2].max())
        base_low = float(low.iloc[-26:-2].min())
        base_volatility = (base_high - base_low) / base_low * 100
        
        if base_volatility > 45.0: # Base too wide/volatile
            return None

        # Turn / Volume Phase Check
        vol_sma10 = float(volume.rolling(window=10).mean().iloc[-1])
        curr_vol = float(volume.iloc[-1])
        
        if vol_sma10 <= 0:
            return None
            
        vol_ratio = curr_vol / vol_sma10
        if vol_ratio < 1.5:
            return None

        # Price Trend Turn Check
        if not (cmp > ema10_val and cmp > sma40_val):
            return None
            
        # Check if the turn is recent (e.g., 10W EMA recently crossed 40W SMA or price just broke out of base)
        # We ensure it's not too extended from the 40W SMA (max 30% above)
        if cmp > sma40_val * 1.30:
            return None
            
        # Score calculation
        score = 50.0
        if vol_ratio >= 2.5: score += 20.0
        elif vol_ratio >= 1.5: score += 10.0
        
        if base_volatility < 30.0: score += 15.0
        elif base_volatility < 40.0: score += 5.0
        
        dist_from_sma40 = (cmp - sma40_val) / sma40_val * 100
        if dist_from_sma40 < 10.0: score += 15.0
        elif dist_from_sma40 < 20.0: score += 10.0
        
        score = round(min(score, 100.0), 1)
        
        buy_price, exit_price, target_price, support, resistance = calculate_trade_levels(df_weekly, cmp)
        
        confidence = f"High ({score}%)" if score >= 80 else f"Medium-High ({score}%)" if score >= 65 else f"Medium ({score}%)"
        
        prev_close = float(close.iloc[-2])
        day_change_pct = round((cmp - prev_close) / prev_close * 100, 2)
        
        base_rec = (
            f"Cupid Strategy: Early Stage-2 Turn from a {base_volatility:.1f}% tight base. "
            f"Volume {vol_ratio:.2f}x 10W avg confirms Smart Money footprint. "
            f"Price above 10W EMA and 40W SMA. "
            f"Buy Range: [₹{buy_price:.2f} to ₹{cmp:.2f}] (Nearest Support is ₹{support:.2f}). "
            f"Set stop loss securely below support at ₹{exit_price:.2f} "
            f"with a target near overhead resistance at ₹{target_price:.2f}."
        )
        recommendation = compute_rich_analysis(df_weekly, symbol, "Cupid Strategy", base_rec)
        ...
        print(f"Cupid scan error for {symbol}: {e}")
        return None
